...
存在特权指令，只能在特权模式下使用；不同特权模式下访存的范围不同

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

访问gate时，从低优先级到高优先级访问，CPL <= DPL门，CPL >= DPL段。
访问段时，从高优先级到低优先级访问，MAX(CPL,RPL) <= DPL段，即较高优先级的若比访问段的优先级要高，则允许访问。
...
发生中断或异常的时候，会判断CPL和DPL门，DPL段的关系
2. 在什么情况下会出现特权级切换？
系统调用，中断发生，结束

3. int指令在ring0和ring3的执行行为有什么不同？
在ring3，会切换栈，并且在栈中压入ss和esp
在ring0，不需要切换栈
...
TSS在切换的时候帮助保存现场的寄存器，恢复时将它们复原到原来的寄存器中

TR是用来保存TSS的段选择子的十六位寄存器。
...
第一句设置CR0的PE标志位，令处理器到保护模式
第二句远跳指令清空预取指令序列，然后移动到CS段描述符，开始执行段描述符指向处代码
